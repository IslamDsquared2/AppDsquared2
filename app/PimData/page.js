...
function PimData() {
    ...
    function PimDataJobLaunch() {
        setIsLoader(true); 
        AvviaJob('1-Akeneo-Import-Attributes').then(() => {
            setTimeout(() => {
                AvviaJob('3-1-1-Akeneo-Differential-Import-Master')
                    ...
                    .catch((error) => {
                        setIsLoader(false);
                        toast.error('Error in completing the second job: ' + error.message);
                        console.error('Error in second job:', error);
                    });
            }, 30000);
        })
        .catch((error) => {
            setIsLoader(false);
            toast.error('Error in completing the first job: ' + error.message);
            console.error('Error in first job:', error);
        });
    }
        ...
        <>
            <div className="w-full h-full">
                ...
                <div className='flex flex-col w-[100vw] h-[calc(100vh-90px)] justify-center items-center'>
                    ...
                    <button className={` m-2 ${isLoader ? 'select-file-label-disabled' : 'select-file-label downloaded'}  `}onClick={() => PimDataJobLaunch()}>
